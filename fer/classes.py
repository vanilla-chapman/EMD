...
class Video(object):
    ...
    def analyze(
        ...
        detector (fer.FER): распознаватель выражений лица
        display (bool): показывать изображения с помощью cv2.imshow
        output (str): csv или pandas
        frequency (int): вывод на каждом n-ном кадре (чем больше число, тем быстрее)
        max_results (int): количество кадров для выполнения вывода перед остановкой
        save_fps (bool): частота вывода = видео fps // save_fps
        video_id (str): имя файла для сохранения
        save_frames (bool): сохраняет кадры в каталог
        save_video (bool): сохраняет выходное видео
        annotate_frames (bool): добавляет метки эмоций
        zip_images (bool): сжимает вывод
        detection_box (dict): dict с ограничивающим прямоугольником для подизображения (xmin, xmax, ymin, ymax)
        lang (str): язык эмоций, который будет показан на видео
        include_audio (bool): указывает, следует ли создавать озвученную версию видео прогнозирования
        size_multiplier (int): увеличивает размер метки эмоций, показанные в видео x(size_multiplier)
        Возвращает:

            data (list):список результатов
